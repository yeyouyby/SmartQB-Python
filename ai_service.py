...
class AIService:
    ...
    def _get_chat_kwargs(self):
        ...
        if (
            ...
            and self.settings.temperature is not None
        ):
            try:
        ...
        if (
            hasattr(self.settings, "max_tokens")
            and self.settings.max_tokens is not None
        ):
            try:
...
你的唯一目标是从混杂的 OCR 文本（可能是 Markdown 格式或包含图片占位符）及可能的附加图片中，提取出结构完整、排版规范的题目。

【极其严格的格式约束】
0. 文本中可能会出现图样标记（如 [[{ima_dont_del_0_1}]]），你必须原封不动地将其保留在它所在的题目文本中，绝对不要修改、删除、转义（包括下划线 _）或添加任何额外字符。
1. 彻底删除最开头的题目序号（如 "1.", "(2)", "一、" 等），绝对不允许保留大题号。
2. 题目内容应该被输出为标准 Markdown 格式。对于数学公式、变量，必须包裹在 $...$ 或 $$...$$ 中（LaTeX格式）。
3. 文本中原有的 Markdown 排版（如加粗、表格）应尽可能保留。
4. 输出的 Content 必须是纯正且合规的 Markdown（包含 LaTeX 公式支持）。
...
{{
    "Content": "标准 Markdown 格式的题目内容（公式使用 $ 或 $$）...",
...
"""
    ...
    def process_slices_with_context(
        ...
        system_content = (
            self._get_system_prompt(is_vision_mode=use_vision)
            + """
...
3. """
            + (
                f"【关键跨页处理】如果某道题目延伸或触碰到了提供的【最后一个辅助切片】（其序号为 {aux_slice_index}），**绝对不要**把它放进 `Questions` 数组中！你必须将这道触碰到最后辅助切片的题目的原始文本放入 `PendingFragment` 字段中，系统会将其与下一个批次的切片合并处理。对于未触碰到该辅助切片的题目，正常放入 `Questions` 数组。"
                if has_aux
                else "【关键跨页处理】当前批次没有辅助切片。请将所有完整或残缺的题目都直接放入 `Questions` 数组中，不需要使用 `PendingFragment`。"
            )
            + """
4. """
            + (
                "当前是文档末尾，没有辅助切片，请将所有识别出的题目都放入 `Questions` 数组，不要放入 `PendingFragment`。"
                if is_last_batch
                else "不要把辅助切片里的新题目和前面的残缺题目强行合并在一起！遇到真正的新题号就立刻切断！"
            )
            + """
5. `NextIndex` 指向下一个批次的主切片起始位置。"""
            + (
                f"当前是最后批次或单切片，必须返回 {last_index_plus_one}。"
                if not has_aux
                else f"应返回本批次辅助切片的序号（即 {aux_slice_index}）。"
            )
            + f"""
...
"""
        ...
        messages = [
            {
                "role": "system",
                "content": "你是一个试题处理专家，必须严格输出JSON格式，不要任何其他废话或Markdown包裹。",
            },
...
"""
        messages = [
            {
                "role": "system",
                "content": "你是一个试题处理与LaTeX专家，必须严格输出JSON格式。",
            },
            {"role": "user", "content": prompt},
        ]
        try:
    ...
    def get_embedding(self, text):
        ...
        try:
            embed_api_key = (
                ...
                if self.settings.embed_api_key
            ...
            embed_model_id = (
                ...
                if self.settings.embed_model_id
    ...
    def chat_with_tools(self, messages, callbacks):
                ...
                "function": {
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            "query": {
                                "type": "string",
                                "description": "高度凝练的学科或知识点关键词（如：导数、圆锥曲线、牛顿第二定律），不要包含冗余的聊天词语",
                            }
                        },
                        "required": ["query"],
                    },
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            "question_ids": {
                                ...
                                "items": {
                                    "type": "integer",
                                    "description": "你要加入试卷的题目ID（通常来自于之前的搜索结果列表中的题号）",
                                },
                            }
                        },
        ...
        for _ in range(max_turns):
            kwargs = self._get_chat_kwargs()
            response = client.chat.completions.create(
                **kwargs, messages=working_messages, tools=tools, tool_choice="auto"
            )
