    ...
    def process_doc_with_layout(
        ...
        try:
            ...
            for page_index in range(total_pages):
                ...
                try:
                    # ==========================================
                    # PASS 1: Layout 图样提取 & Markdown 抽取
                    # ==========================================
                    layout_boxes = []
                    diagrams = []
                    text_regions = []
                    ...
                    try:
                        if layout_predictor is not None:
                            if type(layout_predictor).__name__ == "PPStructureV3":
                                # --- PP-StructureV3 逻辑 ---
                                ...
                                for img_name, img_data in md_images.items():
                                    ...
                                    d_b64 = base64.b64encode(buf.getvalue()).decode(
                                        "utf-8"
                                    )
                    ...
                    # ==========================================
                    # PASS 2: FULL PAGE OCR 文字提取 (仅限 DocLayout-YOLO 模式)
                    # ==========================================
                    ocr_blocks = []
                    annotated_img = img.copy()
                    draw = ImageDraw.Draw(annotated_img)

                    if full_page_markdown is not None:
                        # PP-StructureV3 已经给了完整的 Markdown 页面，不需要再逐个画框匹配，直接使用
                        full_page_text = full_page_markdown
                    else:
                        try:
                            if ocr_engine_type == "Pix2Text" and ocr_engine is not None:
                                ...
                                for block in res:
                                    ...
                                    elif text and block.get("box_2d"):
                        ...
                        for d_idx, d in enumerate(diagrams):
                            ...
                            for t_idx, t in enumerate(ocr_blocks):
                                ...
                                h_overlap = max(
                                    0, min(d_x_max, t_x_max) - max(d_x_min, t_x_min)
                                )
                            ...
                            if best_t_idx != -1:
                                ...
                                nearest_idx = min(
                                    ...
                                    key=lambda i: distance(ocr_blocks[i]),
                                )
                                ocr_blocks[nearest_idx]["text"] = (
                                    marker + ocr_blocks[nearest_idx]["text"]
                                )
                            else:
                                ocr_blocks.append(
                                    {
                    ...
                    page_annotated_b64 = base64.b64encode(buf_anno.getvalue()).decode(
                        "utf-8"
                    )
