...
# Replace the packaging logic in process_doc_with_layout
old_packaging_logic = """                    # 统一排序: 按 y_center 进行从上到下的排序
                    all_elements = []
                    for b in ocr_blocks:
...
new_packaging_logic = """                    # --- NEW LOGIC: Match diagrams to nearest OCR text box above it ---
                    ...
                    slice_obj = {
                        "text": full_page_text,
                        "image_b64": "",  # Whole page crop could go here if needed, but we rely on diagram_map
                        "diagram": None,  # Will be resolved via diagram_map later
...
content = content.replace(old_packaging_logic, new_packaging_logic)
