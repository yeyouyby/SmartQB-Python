...
class WorkerSignals:
    ...
    def emit_started(self, *args, **kwargs):
        for callback in self.started: callback(*args, **kwargs)

    def emit_progress(self, *args, **kwargs):
        for callback in self.progress: callback(*args, **kwargs)

    def emit_finished(self, *args, **kwargs):
        for callback in self.finished: callback(*args, **kwargs)

    def emit_error(self, *args, **kwargs):
        for callback in self.error: callback(*args, **kwargs)
...
class GenericWorker(threading.Thread):
    ...
    def run(self):
        ...
        try:
            result = self.task_func(self.signals.emit_progress, *self.args, **self.kwargs)
